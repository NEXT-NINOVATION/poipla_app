...
// 敵のコンポーネント
class Plastic extends SpriteComponent
  ...
  Plastic({
    ...
    required this.plasticData,
    required Vector2? position,
    required Vector2? size,
  }) : super(sprite: sprite, position: position, size: size) {
    // energyDataから現在の速度を設定します。
    _speed = plasticData.speed;
  ...
  void onCollision(Set<Vector2> intersectionPoints, PositionComponent other) {
    ...
    // if (other is Bullet) {
    //   // もう一方のヒットが弾の場合、弾のレベル*10分だけ体力を減らします。
    //   _hitPoints -= 1;
    // } else if (other is Player) {
    //   // もう一方のヒットがPlayerの場合、破壊する。
    //   destroy();
    // }
    if (other is Player) {
      destroy();
  ...
  void destroy() {
    ...
    //   player.addToScore(plasticData.point);
    // });
